...
<div class="tw-flex tw-items-center tw-gap-4" id="experiencePreview">
    <div class="">
...
<div id="experienceForm" class="db-job-card-table -tw-mx-2 tw-pb-16 tw-hidden"  >
    <table class="tw-px-2">
        <thead>
            <tr>
                <th class="!tw-text-base !tw-font-medium">{{ __('company') }}</th>
                            ...
                            <ul class="dropdown-menu dropdown-menu-end company-dashboard-dropdown"
                                ...
                                <li>
                                    <a href="javascript:void(0)" class="dropdown-item"
                                        onclick="experienceDetail({{ json_encode($experience) }}, '{{ date('d-m-Y', strtotime($experience->start)) }}', '{{ date('d-m-Y', strtotime($experience->end)) }}')">
                                        <x-svg.edit-icon />
                                        {{ __('edit') }}
                                    </a>
                                </li>
                                <li>
                                    <form method="POST"
                                        action="{{ route('candidate.experiences.destroy', $experience->id) }}">
                                        @csrf
                                        @method('Delete')
