                ...
                <table class="table table-hover table-borderless align-middle" id="candidatesTable">
                    <thead class="bg-light">
                        <tr class="text-uppercase text-secondary">
                            ...
                            <th>{{ __('Job Title') }}</th>
                            <th>{{ __('Details') }}</th>
                            <th>{{ __(' Assign to Agent/Recruit Agency') }}</th>
                        </tr>
                    </thead>
                    <tbody>
                        @if ($candidates->count() > 0)
                            @foreach ($candidates as $candidate)
                                <tr>
                                    ...
                                    <td class="fw-bold">{{ $candidate->job->company->user->name ?? __('N/A') }}</td>
                                    ...
                                    <td>
                                        <form action="{{ route('assign.agent', $candidate->candidate_id) }}" method="POST">
                                            @csrf
                                            <select name="admin_id" class="form-select form-select-sm" onchange="this.form.submit()">
                                                <option value="">{{ __('Select Agent') }}</option>
                                                @foreach ($agents as $agent)
                                                    <option value="{{ $agent->id }}"
                                                        {{ $candidate->candidate->admin_id == $agent->id ? 'selected' : '' }}>
                                                        {{ $agent->name }}
                                                    </option>
                                                @endforeach
                                            </select>
