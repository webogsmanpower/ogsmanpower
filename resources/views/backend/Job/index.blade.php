                                ...
                                <tbody>
                                    @if ($jobs->count() > 0)
                                        @foreach ($jobs as $job)
                                                ...
                                                <td>
                                                   ...
                                                   @if (auth()->user()->hasRole('superadmin')) {
                                                    <button type="button" class="btn btn-primary" data-toggle="modal"
                                                        data-target="#assignJobModal_{{$job->id}}" data-job-id="7">
                                                        Assign Roles
                                                    </button>
                                                   @endif
                                                    <a data-toggle="tooltip" data-placement="top"
                                            ...
                                            <div class="modal fade" id="assignJobModal_{{$job->id}}" tabindex="-1" role="dialog" aria-labelledby="assignJobModalLabel"
                                                aria-hidden="true">
                                                <div class="modal-dialog" role="document">
                                                    <div class="modal-content">
                                                        <div class="modal-header">
                                                            <h5 class="modal-title" id="assignJobModalLabel">Assign Roles</h5>
                                                            <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                                                <span aria-hidden="true">&times;</span>
                                                            </button>
                                                        </div>
                                                        <form method="POST" action="{{ route('job.assign-roles', ['id' => $job->id]) }}">
                                                            @csrf
                                                            <div class="modal-body">
                                                                <div class="form-group">
                                                                    <label for="roles">Select Roles</label>
                                                                    <select name="roles[]" id="roles" class="form-control" multiple>
                                                                        <option @if (collect($job->assigned_roles)->contains(fn($item) => Str::contains($item, 'public'))) selected @endif value="public">public</option>
                                                                        @foreach ($roles as $role)
                                                                            <option @if (collect($job->assigned_roles)->contains(fn($item) => Str::contains($item, $role->name))) selected @endif value="{{ $role->name }}">{{ $role->name }}</option>
                                                                        @endforeach
                                                                    </select>
                                                                </div>
                                                            </div>
