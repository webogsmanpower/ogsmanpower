                                        ...
                                        @if ($countries->count() > 0)
                                            @foreach ($countries as $country)
                                                <tr role="row" class="text-center items-center">
                                                    ...
                                                    <td>
                                                        <div>
                                                            <img src="{{ asset($country->image) }}"
                                                                alt="{{ $country->name }}" width="50px" height="50px">
                                                        </div>
                                                    </td>
                                                    ...
                                                    <td>
                                                        <input type="checkbox" class="update-visibility"
                                                            data-id="{{ $country->id }}"
                                                            data-column="candidates_by_country"
                                                            {{ $country->candidates_by_country == 1 ? 'checked' : '' }}>
                                                    </td>

                                                    <td>
                                                        <input type="checkbox" class="update-visibility"
                                                            data-id="{{ $country->id }}" data-column="jobs_by_country"
                                                            {{ $country->jobs_by_country == 1 ? 'checked' : '' }}>
...
@section('script')
    ...
    <script>
        ...
        $(document).on('change', '.update-visibility', function() {
            var $checkbox = $(this);
            var countryId = $checkbox.data('id');
            var column = $checkbox.data('column');
            var value = $checkbox.prop('checked') ? 1 : 0; // Set value to 1 if checked, otherwise 0

            // Add a loading indicator next to the checkbox
            var loader = $('<span class="loader">Updating...</span>');
            $checkbox.closest('td').append(loader); // Add loader in the same cell

            // Ensure we only send a request for one checkbox at a time (prevents double updating)
            if (!$checkbox.hasClass('updating')) {
                $checkbox.addClass('updating'); // Mark this checkbox as being updated

                // Send the AJAX request to update the column
                $.ajax({
                    url: '{{ route('country.toggleVisibility') }}', // URL for the route
                    method: 'POST',
                    data: {
                        _token: '{{ csrf_token() }}',
                        id: countryId,
                        column: column, // Either 'candidates_by_country' or 'jobs_by_country'
                        value: value
                    },
                    success: function(response) {
                        if (response.status === 'success') {
                            // Successfully updated, remove the loader and 'updating' class
                    ...
                    error: function() {
                        ...
                        $checkbox.removeClass('updating');
                        loader.remove(); // In case of error, remove loader as well
                    }
                });
            }
        });
