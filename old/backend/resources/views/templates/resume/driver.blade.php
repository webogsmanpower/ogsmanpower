...
<head>
    ...
    <title>Driver Resume - {{ $resumeData['first_name'] ?? '' }} {{ $resumeData['last_name'] ?? '' }}</title>
    <style>
        @page {
            margin: 0;
            padding: 0;
...
<body>
    ...
    <table style="border: none;">
        <tr>
            <td style="vertical-align: top; width: 70%; padding: 0;">
                <table style="border: none;">
                    <tr>
                        <td class="table-label">{!! getLabel('date', $locale, $labels) !!}</td>
                        <td class="table-value" style="width: 75%;">{{ date('d/m/Y') }}</td>
                    </tr>
                    <tr>
                        <td class="table-label">{!! getLabel('expected_location', $locale, $labels) !!}</td>
                        <td class="table-value" style="width: 75%;">{{ $expectedLocation }}</td>
                    </tr>
                </table>
            </td>
            <td style="vertical-align: top; width: 30%; text-align: right; padding: 0;">
                ...
                @if($profilePhoto)
                    <img src="{{ $profilePhoto }}" style="width: 100%; height: 100%; object-fit: cover;" alt="Profile Photo">
                @else
    ...
    <table style="margin-top: 20px;">
        <tr>
            <td class="table-label">{!! getLabel('full_names', $locale, $labels) !!}</td>
            <td class="table-value" style="width: 35%;">
                {{ $resumeData['first_name'] ?? '' }} {{ $resumeData['last_name'] ?? '' }}
            </td>
            <td class="table-label" style="width: 15%;">{!! getLabel('passport_no', $locale, $labels) !!}</td>
            <td class="table-value" style="width: 25%;">
                {{ $resumeData['documents']['passport_number'] ?? $resumeData['basic_information']['passport_number'] ?? '' }}
            </td>
        </tr>
        <tr>
            <td class="table-label">{!! getLabel('nationality', $locale, $labels) !!}</td>
            <td class="table-value">
                {{ $basicInfo['nationality'] ?? $basicInfo['country'] ?? '' }}
            </td>
            <td class="table-label">{!! getLabel('marital_status', $locale, $labels) !!}</td>
            <td class="table-value">
                {{ $basicInfo['marital_status'] ?? '' }}
            </td>
        </tr>
        <tr>
            <td class="table-label">{!! getLabel('age', $locale, $labels) !!}</td>
            <td class="table-value">{{ $age }}</td>
            <td class="table-label">{!! getLabel('date_of_birth', $locale, $labels) !!}</td>
            <td class="table-value">{{ $formatDate($dob) }}</td>
        </tr>
        <tr>
            <td class="table-label">{!! getLabel('address', $locale, $labels) !!}</td>
            <td class="table-value" colspan="3" style="width: 75%;">{{ $fullAddress }}</td>
        </tr>
    </table>

    <!-- License Block -->
    <div class="section-header">
        {!! getLabel('driver_license_record', $locale, $labels) !!}
    </div>
    <table style="margin-top: -1px;">
        <tr>
            <td class="table-label">{!! getLabel('license_no', $locale, $labels) !!}</td>
            <td class="table-value" style="width: 30%;">{{ $resumeData['license_number'] ?? '' }}</td>
            <td class="table-label" style="width: 15%;">{!! getLabel('issuing_country', $locale, $labels) !!}</td>
            <td class="table-value" style="width: 30%;">{{ $resumeData['license_issuing_country'] ?? '' }}</td>
        </tr>
        <tr>
            <td class="table-label">{!! getLabel('issuing_authority', $locale, $labels) !!}</td>
            <td class="table-value">{{ $resumeData['license_issuing_authority'] ?? '' }}</td>
            <td class="table-label">{!! getLabel('license_type', $locale, $labels) !!}</td>
            <td class="table-value">{{ $resumeData['license_type'] ?? '' }}</td>
        </tr>
        <tr>
            <td class="table-label">{!! getLabel('license_expiry', $locale, $labels) !!}</td>
            <td class="table-value">{{ $formatDate($resumeData['license_expiry_date'] ?? $resumeData['license_expiry'] ?? '') }}</td>
            <td class="table-label">{!! getLabel('clean_record', $locale, $labels) !!}</td>
            <td class="table-value">
                {{ ($resumeData['has_clean_driving_record'] ?? false) ? getLabel('yes', $locale, $labels) : getLabel('no', $locale, $labels) }}
            </td>
        </tr>
        <tr>
            <td class="table-label">{!! getLabel('accident_free_years', $locale, $labels) !!}</td>
            <td class="table-value" colspan="3">{{ $resumeData['accident_free_years'] ?? '0' }}</td>
        </tr>
    </table>

    <!-- Employment Block -->
    <div class="section-header">
        {!! getLabel('previous_employment_abroad', $locale, $labels) !!}
    </div>
    <table style="margin-top: -1px;">
        <tr class="bg-gray">
            <th class="border text-center" style="width: 25%;">{!! getLabel('period', $locale, $labels) !!}</th>
            <th class="border text-center" style="width: 45%;">{!! getLabel('position', $locale, $labels) !!}</th>
            <th class="border text-center" style="width: 30%;">{!! getLabel('country', $locale, $labels) !!}</th>
        </tr>
        @php
        ...
        <tr>
            <td class="border text-center">
                {{ $formatDate($exp['start_date'] ?? '') }} - {{ !empty($exp['end_date']) ? $formatDate($exp['end_date']) : 'Present' }}
            </td>
            <td class="border text-center font-bold">{{ $exp['role_title'] ?? $exp['job_title'] ?? '' }}</td>
            <td class="border text-center">{{ $exp['location'] ?? $exp['country'] ?? '' }}</td>
        </tr>
        @empty
        <tr>
    ...
    <!-- Education Block -->
    <div class="section-header">
        {!! getLabel('educational_qualifications', $locale, $labels) !!}
    </div>
    <table style="margin-top: -1px;">
        @php
        ...
        @forelse($edu as $e)
        <tr>
            <td class="border text-center" style="width: 20%;">{{ $e['graduation_year'] ?? $e['year'] ?? '' }}</td>
            <td class="border text-center font-bold" style="width: 40%;">{{ $e['degree_title'] ?? $e['degree'] ?? $e['qualification'] ?? '' }}</td>
            <td class="border text-center" style="width: 40%;">{{ $e['institution_name'] ?? $e['school'] ?? '' }}</td>
        </tr>
        @empty
        <tr>
    ...
    <!-- Languages Block -->
    <div class="section-header">
        {!! getLabel('languages', $locale, $labels) !!}
    </div>
    <table style="margin-top: -1px;">
        @php
        ...
        @forelse($langs as $l)
        <tr>
            <td class="border font-bold" style="width: 30%;">{{ strtoupper($l['language_name'] ?? '') }}</td>
            <td class="border font-bold">{{ $l['proficiency_level'] ?? $l['level'] ?? '' }}</td>
        </tr>
        @empty
        <tr>
    ...
    <!-- Remarks -->
    <div class="section-header">
        {!! getLabel('remarks', $locale, $labels) !!}
    </div>
    <table style="margin-top: -1px;">
        <tr>
            <td class="border text-center font-bold" style="padding: 20px;">
                {!! getLabel('ready_to_work', $locale, $labels) !!}
            </td>
        </tr>
    </table>
